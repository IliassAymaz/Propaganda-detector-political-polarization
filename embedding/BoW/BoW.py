...
def get_word_map(bow):
    return {w : i+1 for (i, w) in enumerate(bow)}
...
def document_to_vector2(sequence, word_map):
    words = re.findall(r'\w+', sequence.lower())
    return [word_map[w] for w in words]
...
def main():
    ...
    save_to_file('words.txt', uniques)
